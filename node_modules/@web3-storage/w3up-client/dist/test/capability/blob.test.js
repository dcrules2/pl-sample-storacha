...
export const BlobClient = Test.withContext({
    'should store a blob': async (assert, { connection, provisionsStorage, allocationsStorage }) => {
        ...
        assert.deepEqual(await allocationsStorage.exists(space.did(), multihash.bytes), {
            ok: true,
        });
        assert.deepEqual(multihash.bytes, bytesHash.bytes);
    },
    'should list stored blobs': async (assert, { connection, provisionsStorage }) => {
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
                access: connection,
        ...
        assert.deepEqual(multihash.bytes, bytesHash.bytes);
        const { results: [entry], } = await alice.capability.blob.list();
        assert.deepEqual(entry.blob.digest, bytesHash.bytes);
        assert.deepEqual(entry.blob.size, bytes.length);
    },
    'should remove a stored blob': async (assert, { connection, provisionsStorage }) => {
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
                access: connection,
    ...
    'should get a stored blob': async (assert, { connection, provisionsStorage }) => {
        const alice = new Client(await AgentData.create(), {
            // @ts-ignore
            serviceConf: {
                access: connection,
